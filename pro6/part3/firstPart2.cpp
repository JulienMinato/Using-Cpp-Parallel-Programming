...
const char *			CL_FILE_NAME = { "firstMultReduce.cl" };
...
main( int argc, char *argv[ ] )
{
	...
	// MULT_REDUCE
	status = clSetKernelArg( kernel, 2, LOCAL_SIZE*sizeof(float), NULL ); // local 'prods' array - one per work-item
	if( status != CL_SUCCESS )
		fprintf( stderr, "clSetKernelArg failed (3)\n" );
	...
	fprintf( stderr, "%d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
