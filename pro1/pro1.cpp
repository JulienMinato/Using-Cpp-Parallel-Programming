...
main( int argc, char *argv[ ] )
{
    ...
    for (int t = 0; t < NUMTRIES; t++)
	{
		...
		for (int n = 0; n < NUMTRIALS; n++)
		{
			...
			// case A: If d is less than 0., then the circle was completely missed. (Case A) Continue on to the next trial in the for-loop.
			if (d < 0) {
				continue; //A
			}
    ...
    //printf("Threads: %d\t Trials: %d\t Probability:  %8.2lf\t MegaTrialsPerSecond: %8.2lf\t\n", NUMT, NUMTRIALS, currentProb, maxPerformance);
